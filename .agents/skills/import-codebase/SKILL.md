...
- Existing intent: `README`, `docs/`, `AGENTS.md`, `CLAUDE.md`, `PRODUCT.md`, ADRs,
  `wiki/overview.md` if present, design-system docs, and tests that reveal intended behaviour.
...
3. Include the overview impact: whether this capability changes the whole-product map,
   main journeys, capabilities, rules, boundaries, or key decisions.
4. Tick the capability `[x]` in `intake/import-inventory.md`.
5. Run `node scripts/proposal-lint.mjs` and `node scripts/proposal-traceability-lint.mjs`.
...
- Whole-product overview gaps that need the owner to confirm.
