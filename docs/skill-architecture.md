...
The native Stop hook is the automation primitive for deterministic loops.
...
- Explicit overview, index, and log maintenance when a workflow changes the product wiki.
...
- Kiro: durable steering/spec files plus lightweight recurring work.
...
`wiki/overview.md` is the first page in that wiki: the whole-product map.
`wiki/index.md` is the linked catalog, and `wiki/log.md` is the change record.
...
The native Stop hook does not make product decisions.
It runs deterministic checks and tells the main agent when a reconciliation skill should be used.
