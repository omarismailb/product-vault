...
- From Kiro: steering files, recurring checks, and the idea that agent behaviour should be shaped by durable project instructions.
- From Tessl: skills need lifecycle, versioning, evaluation, and distribution discipline.
- From Claude Code and Codex: skills as the normal workflow primitive, the Stop hook plus scripts for deterministic checks, and subagents as optional reviewers rather than mandatory ceremony.
...
Product Wiki is an early, production-shaped harness with a deliberately narrow claim, hardening as it is used against real changes.
...
- Explicit upgrades, because silently changing scripts, settings, and agent rules in installed repos is unsafe.
