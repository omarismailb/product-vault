...
5. `wiki/overview.md` is reviewed or updated so the whole-product map stays current.
6. The compiler skill turns the wiki change into design decisions, checks, implementation steps, code, and verification.
7. Important code and tests get `PW:` anchors that point back to the relevant wiki IDs.
8. The native Stop hook and routine runner execute deterministic loops.
9. Reconcile skill links implementation evidence back to the wiki and raises proposals for judgement calls.
...
|---|---|---|
| Product overview | Product repo | Reviewed when active wiki units change |
...
| Schemas/scripts/agent settings | Product Wiki | Updated through explicit upgrade |
...
Scripts, CI, checks, and the native Stop hook enforce deterministic parts.
